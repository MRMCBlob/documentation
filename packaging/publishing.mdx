...
valid signature ([signing.md](signing)) and go through the same sandbox/permission
setup ([../platform/permissions.md](../platform/permissions)).
...
- The publisher key is pinned to the `app.id` ([signing.md](signing)); future updates
...
Bump `version`/`build` ([manifest.md](manifest)), rebuild, and `zelto publish` again.
The store rolls the update to users; private app data is preserved
([zap-format.md](zap-format)).
...
- [ ] Tested on device ([../getting-started/run-on-device.md](../getting-started/run-on-device))

## See also

- [../tooling/cli.md](../tooling/cli) — `publish` flags.
- [signing.md](signing) · [zap-format.md](zap-format)
