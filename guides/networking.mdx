...
> Declare `network = true` in `zelto.toml` ([../packaging/manifest.md](../packaging/manifest)).
...
Combine with state + effect ([state-management.md](state-management)):
...
([../getting-started/simulator.md](../getting-started/simulator)).
...
Downloaded files land in app storage — see [storage.md](storage).
...
See [../api-reference/c/system.md](../api-reference/c/system).
...
- Respect the user's connection (metered/cellular); avoid large background transfers
  without a declared background capability ([background-tasks.md](background-tasks)).
