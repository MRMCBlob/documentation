...
3. **Layout:** a single-pass stack/flex layout computes frames
   ([../guides/layout.md](../guides/layout)).
...
them ([../ui/components/list.md](../ui/components/list)).
...
- `useAnimatedValue` updates a property **on the render thread**, bypassing rebuilds —
  used for drags and continuous motion ([../guides/animation.md](../guides/animation)).
...
- Heavy app compute belongs in worker threads / native modules, not the app loop
  ([../api-reference/conventions.md](../api-reference/conventions)).
...
- [compositor-internals.md](compositor-internals) · [../api-reference/c/ui.md](../api-reference/c/ui)
