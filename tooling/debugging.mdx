...
([../zelto-script/stdlib.md](../zelto-script/stdlib)).
...
([../zelto-script/runtime.md](../zelto-script/runtime)).
...
| Symptom | Look at |
|---|---|
| UI not updating | Are you mutating state without `setState`/`z_invalidate`? ([../guides/state-management.md](../guides/state-management)) |
| Jank while scrolling | Heavy `row` builders; use `List` recycling ([../ui/components/list.md](../ui/components/list)); profile ([profiling.md](profiling)) |
| Permission errors | Declared in manifest? Granted at runtime? ([../platform/permissions.md](../platform/permissions)) |
| Crash on launch | Check `zelto logs`; validate manifest with `zelto doctor` |
| Native module fails | ABI/build errors — `zelto build --verbose` ([../guides/interop-c-and-script.md](../guides/interop-c-and-script)) |
...
- [profiling.md](profiling) — performance.
- [simulator.md](simulator) — inner-loop dev.
