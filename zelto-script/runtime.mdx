...
app doesn't parse source at launch ([../packaging/zap-format.md](../packaging/zap-format)).
...
- Don't block it: use `async/await` for I/O, and offload heavy compute to a **native C
  module** ([../guides/interop-c-and-script.md](../guides/interop-c-and-script)).
...
- **Memoize** expensive derivations (`useMemo`) and keep `List`/`Grid` `row` builders
  cheap ([../ui/components/list.md](../ui/components/list)).
...
- App memory and background time are bounded by the system
  ([../guides/background-tasks.md](../guides/background-tasks)).
...
`zelto run`/`zelto debug` — see [../tooling/debugging.md](../tooling/debugging) and
[../tooling/profiling.md](../tooling/profiling).
