...
- `useState` holds state; calling `setCount` re-runs the function and the framework
  updates only what changed. See [State management](/guides/state-management).
- `.font(...)` and `.filled()` are **modifiers** (chained styling) — see
  [Styling & theming](/guides/styling-theming).
...
[Simulator](/getting-started/simulator).
...
- `Z_APP(StateType, bodyFn)` generates the entry point. See
  [C UI reference](/api-reference/c/ui).
...
See [.zap format](/packaging/zap-format) and
[Run on device](/getting-started/run-on-device) to install it on a phone.
